...
if __name__ == "__main__":
    ...
    out_dir = os.path.join(
        root_output_dir,
        f"task_3_peak_classification/supervised_model_outputs/ab_initio/{model_name}",
    )
    ...
    model_dir = os.path.join(
        root_output_dir,
        f"task_3_peak_classification/supervised_models/ab_initio/{model_name}",
    )
    ...
    test_dataset = PeaksEmbeddingsDataset(
        peaks_h5, elements_tsv, modes[eval_mode], classes
    )

    model = CNNSequenceBaselinePredictor(
        emb_channels,
        hidden_channels,
        kernel_size,
        seq_len,
