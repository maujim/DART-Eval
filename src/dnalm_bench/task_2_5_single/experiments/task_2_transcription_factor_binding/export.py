...
if __name__ == "__main__":
    dataset_file = os.path.join(
        work_dir, "task_2_footprinting/processed_data/footprint_dataset_350_v1.txt"
    )
    ...
    for motif in np.unique(seq_data["motif"]):
        if motif == "raw":
            continue
        true_seqs = seq_data.loc[
            (seq_data["motif"] == motif) & (seq_data["type"] == "true")
        ]
        shuf_seqs = seq_data.loc[
            (seq_data["motif"] == motif) & (seq_data["type"] == "shuffled")
        ]
    ...
    true_motif_seqs = np.array(
        [[one_hot_encode(x) for x in y] for y in true_motif_string_seqs]
    )
    shuff_motif_seqs = np.array(
        [[one_hot_encode(x) for x in y] for y in shuff_motif_string_seqs]
    )
    ...
    with h5py.File(out_path, "w") as f:
        ...
        f.create_dataset(
            name="positive", data=true_motif_seqs, dtype=np.uint8, compression="gzip"
        )
        f.create_dataset(
            name="shuffled", data=shuff_motif_seqs, dtype=np.uint8, compression="gzip"
        )
