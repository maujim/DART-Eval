...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Given a list of motif-inserted sequences and their likelihoods, determine model performance on each motif"
    )
    parser.add_argument(
        "--input_seqs",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="tsv file containing model likelihoods",
    )
    parser.add_argument(
        "--output_file",
        type=str,
...
def get_accuracies(likelihood_dict):
    ...
    for motif in likelihood_dict:
        ...
        diffs = [
            curr_dict["true"][x] - curr_dict["shuffled"][x] for x in range(num_seqs)
        ]
...
def get_pvals(likelihood_dict):
    ...
    for motif in likelihood_dict:
        curr_dict = likelihood_dict[motif]
        pval = wilcoxon(
            curr_dict["true"], curr_dict["shuffled"], alternative="greater"
        )[1]
...
def main():
    ...
    final_df = pd.DataFrame(
        [accuracies, pvals, background_accs],
        index=["Accuracy", "P-Value", "Accuracy vs Background"],
    ).T
