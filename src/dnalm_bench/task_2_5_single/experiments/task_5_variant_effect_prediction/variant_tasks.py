...
def compute_change(filtered_variants_df, switch=False):
    if switch:
        filtered_variants_df["llm_logfc"] = (
            filtered_variants_df["allele1_scores"]
            - filtered_variants_df["allele2_scores"]
        )
    else:
        filtered_variants_df["llm_logfc"] = (
            filtered_variants_df["allele2_scores"]
...
def sig_ctrl_variants_Afr_CaQTLs(scores_data_path):
    afr_caqtls_data_path = os.path.join(
    ...
    if afr_caQTLs_df.shape[0] == scores.shape[0]:
        likelihoods_data = pd.merge(
            afr_caQTLs_df, scores, on=["chr_hg38", "pos_hg38", "allele1", "allele2"]
        )
        filtered_var_afr_caQTLs_df = likelihoods_data[
            (likelihoods_data["IsUsed"] == True)
            & (likelihoods_data["in_peaks"] == True)
        ].copy(deep=True)

        print("unique label values", filtered_var_afr_caQTLs_df["label"].value_counts())

        filtered_var_afrcaqtls_df_sig = filtered_var_afr_caQTLs_df[
            (filtered_var_afr_caQTLs_df["label"] == 1)
        ]
        filtered_var_afrcaqtls_df_ctrl = filtered_var_afr_caQTLs_df[
            (filtered_var_afr_caQTLs_df["label"] == 0)
        ]
...
def variants_Yoruba_LCL_dsQTLs(scores_data_path):
    yoruba_dsqtls_data_path = os.path.join(
        work_dir,
    ...
    scores = likelihoods[
        ["var.chrom", "var.pos", "var.allele1", "var.allele2"] + columns
    ]
    if yoruba_dsQTLs_df.shape[0] == scores.shape[0]:
        likelihoods_data = pd.merge(
            yoruba_dsQTLs_df,
            scores,
            on=["var.chrom", "var.pos", "var.allele1", "var.allele2"],
        )
        filtered_var_yoruba_dsQTLs_df = likelihoods_data[
            likelihoods_data["var.isused"] == True
        ].copy(deep=True)

        print(
            "unique label values",
            filtered_var_yoruba_dsQTLs_df["var.label"].value_counts(),
        )

        filtered_var_yoruba_dsqtls_df_sig = filtered_var_yoruba_dsQTLs_df[
            (filtered_var_yoruba_dsQTLs_df["var.label"] == 1)
        ]
        filtered_var_yoruba_dsqtls_df_ctrl = filtered_var_yoruba_dsQTLs_df[
            (filtered_var_yoruba_dsQTLs_df["var.label"] == -1)
        ]
...
def sig_ctrl_variants_Afr_CaQTLs_probed_counts(counts_data_path):
    ...
    filtered_var_afr_caQTLs_df = counts_data[counts_data["IsUsed"] == True].copy(
        deep=True
    )
    filtered_var_afr_caQTLs_df["llm_logfc"] = (
        filtered_var_afr_caQTLs_df["allele2_scores"]
        - filtered_var_afr_caQTLs_df["allele1_scores"]
    )

    print("unique label values", np.unique(filtered_var_afr_caQTLs_df["label"]))
    filtered_var_afrcaqtls_df_sig = filtered_var_afr_caQTLs_df[
        (filtered_var_afr_caQTLs_df["label"] == 1)
        & (-np.log10(filtered_var_afr_caQTLs_df["pval"]) >= 3)
    ]
    filtered_var_afrcaqtls_df_ctrl = filtered_var_afr_caQTLs_df[
        (filtered_var_afr_caQTLs_df["label"] == 0)
        & (-np.log10(filtered_var_afr_caQTLs_df["pval"]) < 3)
