...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Text file containing input negative elements which will be footprinted",
    )
    parser.add_argument("--output_file", type=str, required=True, help="Output file")
    parser.add_argument(
        "--meme_file", type=str, required=True, help="Meme file containing motif PWMs"
    )
...
def get_all_inserts_for_seq(seq, motif_dict):
    ...
    for motif in motif_dict:
        seq_dict[motif] = [
            insert_motif(seq, motif_dict[motif]),
            insert_motif(seq, shuffle(motif_dict[motif])),
        ]
    return seq_dict
...
def compile_seqs(seq_file, motif_dict):
    ...
    for row, line in enumerate(seq_file_obj):
        ...
        for motif in curr_seq_dict:
            overall_seq_dict[str(row) + "_" + motif + "_forward_true"] = curr_seq_dict[
                motif
            ][0]
            overall_seq_dict[str(row) + "_" + motif + "_forward_shuffled"] = (
                curr_seq_dict[motif][1]
            )
            overall_seq_dict[str(row) + "_" + motif + "_reverse_true"] = (
                reverse_complement(curr_seq_dict[motif][0])
            )
            overall_seq_dict[str(row) + "_" + motif + "_reverse_shuffled"] = (
                reverse_complement(curr_seq_dict[motif][1])
            )
